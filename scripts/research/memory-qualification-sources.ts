...
export function snapshotMemoryQualification(directory: string) {
  ...
  const patterns = [
    ...
    "src/agent/durable-memory.ts",
    "src/agent/context-manager.ts",
    "src/engine/commands/memory.ts",
    ...
    "src/net/memory-mcp-resources.ts",
    "extensions/langgraph-store/*.ts",
    "extensions/langgraph-store/package.json",
    "extensions/langgraph-store/bun.lock",
    "dashboard/src/components/MemoryWorkspace.tsx",
    "dashboard/src/lib/memory-service.ts",
    ...
    "scripts/research/memory-coding-graders.ts",
