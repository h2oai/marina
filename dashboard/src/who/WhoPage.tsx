...
/**
 * True at or below `NARROW_LAYOUT_MAX_WIDTH` (phones, narrow side panels). The
 * column layout itself is CSS (Tailwind `sm:` breakpoints); this only drives
 * the few things CSS cannot — the sigil's rendered size and the copy button's
 * label. Safe without `matchMedia` (SSR, old jsdom): defaults to wide.
 */
...
export function WhoPage() {
  ...
  // The SPA shell ships a generic <title>; give every profile its own so tabs,
  // history and screen readers name the entity.
  useEffect(() => {
...
function WhoLanding() {
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-6 px-4 py-16 sm:px-6">
...
function WhoProfile({ name }: { name: string }) {
  ...
  if (error || !data) {
    return (
      <div className="mx-auto flex min-h-screen w-full max-w-3xl flex-col items-center justify-center gap-3 px-4 sm:px-6">
  ...
  return (
    <main
      data-testid="who-profile"
      className="mx-auto min-h-screen w-full max-w-3xl overflow-x-hidden px-4 pb-16 pt-8 sm:px-6 [&_img]:h-auto [&_img]:max-w-full [&_svg]:max-w-full"
    >
      <Nav />
...
function Identity({ profile }: { profile: EntityProfile }) {
  ...
  return (
    <motion.section
      ...
      data-testid="who-identity"
      // Single column up to the `sm` breakpoint (640px): sigil, then text, then
      // the rank/standing pills as a row. Side-by-side above it.
      className="glass-panel mb-6 flex flex-col items-start gap-4 p-4 sm:flex-row sm:gap-5 sm:p-5"
    >
      <Sigil name={identity.name} size={narrow ? 64 : 88} />
      <div className="min-w-0 w-full flex-1">
        ...
        <div className="mt-3 grid gap-1 break-all text-[10px] text-text-dim sm:grid-cols-2">
        ...
        <button
          type="button"
          aria-label={copied ? "Canonical link copied" : "Copy canonical profile link"}
        ...
        >
          {copied ? "Link copied" : narrow ? "Copy link" : "Copy canonical profile link"}
        </button>
      </div>
      <div className="flex w-full shrink-0 flex-row items-end gap-6 sm:w-auto sm:flex-col sm:gap-2 sm:text-right">
      ...
      <dl className="grid gap-y-2 text-[13px] sm:grid-cols-[max-content_1fr] sm:gap-x-4">
        {lines.map((l) => (
          <div key={l.label} className="contents">
            ...
            <dd className="min-w-0 whitespace-pre-wrap break-words text-text">{l.value}</dd>
          </div>
        ))}
      </dl>
    </motion.section>
  ...
  return (
    <article className="glass-panel p-5">
      <header className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
        <h3 className="min-w-0 break-words font-display text-base text-text-bright">
          {entry.title}
        </h3>
      ...
      {entry.body && (
        <p className="mb-3 whitespace-pre-wrap break-words text-[13px] text-text">{entry.body}</p>
      )}
      <footer className="flex flex-wrap items-center gap-x-3 gap-y-1 break-all text-[10px] text-text-dim">
      ...
      <dl className="space-y-1 text-[12px]">
        {rows.map((r) => (
          <div key={r.label} className="flex justify-between gap-2">
            ...
            <dd className="break-all text-right font-mono text-text-bright">{r.value}</dd>
          </div>
        ))}
      </dl>
    </div>
          ...
          {list.map((c) => (
            <li key={c.name} className="flex items-center justify-between gap-2">
              <a
                ...
                className="min-w-0 break-words text-[13px] text-text hover:text-primary"
              >
                {c.name}
              </a>
