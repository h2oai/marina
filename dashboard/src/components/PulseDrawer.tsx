...
export function PulseDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <AnimatePresence>{open && <PulseDrawerPanel key="panel" onClose={onClose} />}</AnimatePresence>
  );
}

function PulseDrawerPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-px border-b border-border bg-border text-center text-[10px]">
        ...
        <div className="bg-bg-card p-2">
          <strong
            className={failures ? "block text-base text-danger" : "block text-base text-success"}
          >
            {failures}
          </strong>
          observed failures
