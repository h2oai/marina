...
- **Flywheel sandbox direction** (`docs/design/code-flywheel-execution-plan.md`): the accepted Phase-6 boundary is one durable Flywheel sandbox per Marina entity. Marina owns identity, policy, approvals, Code Mode, artifacts, and audit; Flywheel owns isolated execution, persistent guest disk, hibernate/resume, and publish. The shipped `FlywheelManager` is currently in-memory and MCP-only. Next: durable reconciliation and `code sandbox` lifecycle, then explicit sandbox runtime routing, then project materialization. Never silently fall back from requested sandbox execution to host execution or imply host/guest file coherence.
