...
- **Coverage**: single-agent routing (`/v1/chat/completions` agents mode, `/v1/responses`, Ollama routes), the verified fast path, and open/panel fan-out (per-target spans under one shared trace). Passthru mode and passthru fallbacks proxy upstream untraced. Every agent-routed response returns its traced id in `x-request-id` — the id for `trace show <id>`.
