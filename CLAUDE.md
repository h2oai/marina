...
- **`trace` command** (rank 0): `list/show/stats/compare/dataset [verify]/advise/eval/judge/judgments`. `trace dataset verify` round-trips the eval-json export and replays `replayTraceDataset`, reporting schema validity + counts + drift. `GET /api/traces` (authenticated) serves formats `native`, `eval-json` (`marina.trace.dataset.v1`), `otlp-json` (pull-only OTLP; no collector push). Dashboard Admin → Traces tab (`dashboard/src/components/TraceExplorer.tsx`).
