...
- **Silo boundary**: durable records live in `notes` under `entity_name = memory:<principalId>` and are hidden from legacy surfaces (`isServiceMemoryNote`); legacy notes are invisible to the service. The prompt reads legacy only. Convergence (one retrieval surface; legacy commands as adapters) is the Phase-1 memory goal — do not add a third store.
