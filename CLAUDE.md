...
- **`trace` command** (rank 0): `list/show/stats/compare/dataset/advise/eval/judge/judgments`. `GET /api/traces` (authenticated) serves formats `native`, `eval-json` (`marina.trace.dataset.v1`), `otlp-json` (pull-only OTLP; no collector push). Dashboard Admin → Traces tab (`dashboard/src/components/TraceExplorer.tsx`).
