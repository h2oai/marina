...
The ECR registry account (`353750902984`) and repo name (`h2oai-marina`) are constants in the workflow's `env:` block — the image is `353750902984.dkr.ecr.us-east-1.amazonaws.com/h2oai-marina`.

**Cross-account ECR:** the OIDC role lives in `906013726799` but the ECR repo lives in `353750902984` (the h2o-ecr account). This works because that repo's **resource policy grants the whole AWS org** push/pull — no second assume-role needed. `amazon-ecr-login` logs in against the `353750902984` registry via its `registries:` input.

In **AWS**:

- **ECR repo** `h2oai-marina` in account `353750902984` (managed in `terraform/aws/internal-deployments/ecr`), with the default org-wide repository policy.
