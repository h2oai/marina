...
Spike (2026-09-24): wire format, both backends, the three policies, the pi tool gate, `/v1/decisions`, events and readiness. Followed by spawn-time routing and the route table, the task-submission verifier, owner approvals for `ask`, TypeSafe compatibility (`/v1/systemone`, `typesafe` preset), gate authorization context, calibration-aware thresholds, and the Admin → Ops → Decisions view. Open: a benchmark A/B of gate verdicts across backends before recommending any default (`scripts/qualify-decisions.ts`).
