...
Spike (2026-09-24): wire format, both backends, the three policies, the pi tool gate, `/v1/decisions`, events and readiness. Spawn-time routing and the task-submission verifier followed. Not yet wired: an approver for gate `ask` verdicts and dashboard views of `agent_decision`.
